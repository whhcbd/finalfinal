...
export class Analyzer {
    ...
    #ongoingWorkPromises = [];
    #accessedPackageJsonFiles = new Set();
    #resolvedWorkspacesConfigs = [];
    #agent;
    #logger;
    ...
    async getPackageJson(packageDir) {
        this.#accessedPackageJsonFiles.add(pathlib.join(packageDir, 'package.json'));
        return this.#packageJsonReader.read(packageDir);
    }
    #makeRelevantConfigGlobGroups() {
        return [
    ...
    async #handleWireitScript(placeholder, packageJson, syntaxInfo, wireitConfig) {
        ...
        const { dependencies, encounteredError: dependenciesErrored } = await this.#processDependencies(placeholder, packageJson, syntaxInfo);
        let command;
    ...
    async #processDependencies(placeholder, packageJson, scriptInfo) {
        const dependencies = [];
        ...
        for (const maybeUnresolved of children) {
            ...
            const result = await this.#resolveDependency(packageJson, placeholder, unresolved);
            if (!result.ok) {
                encounteredError = true;
                placeholder.failures.push(result.error);
                continue;
        ...
        for (const [uniqueKey, { script, unresolved, cascade },] of resolvedScripts.entries()) {
                ...
                for (const failure of failures) {
                    if (failure.reason === 'script-not-found') {
                        ...
                        placeholder.failures.push({
                            ...
                            diagnostic: {
                                severity: 'error',
                                message: `Cannot find script named ${JSON.stringify(script.name)} in package "${script.packageDir}"`,
                                location: {
                                    file: packageJson.jsonFile,
                                    range: { offset, length },
                                },
                        ...
                        placeholder.failures.push({
                            ...
                            diagnostic: {
                                severity: 'error',
                                message: `package.json file missing: "${pathlib.join(script.packageDir, 'package.json')}"`,
     ...
     */
    async #resolveDependency(packageJson, contextScript, dependencySpecifier) {
                ...
                error: {
                    ...
                    diagnostic: {
                        ...
                        location: {
                            ...
                            file: packageJson.jsonFile,
                            range: {
                                offset: dependencySpecifier.offset +
                                    1 +
        ...
        const { package: parsedPackage, script: parsedScript, inverted, } = parsed.value;
        const scriptName = this.#resolveScriptName(parsedScript, contextScript);
        if (!scriptName.ok) {
            return scriptName;
        }
        const packageDirs = await this.#resolvePackageDirs(contextScript, packageJson, dependencySpecifier, parsedPackage, scriptName.value);
        if (!packageDirs.ok) {
            return packageDirs;
        }
        return {
    ...
    #resolveScriptName(script, context) {
        if (script.kind === 'name') {
        ...
        script;
        throw new Error(`Unexpected parsed script format: ${JSON.stringify(script)}`);
    }
    async #resolvePackageDirs(contextScript, packageJson, dependencySpecifier, parsedPackage, scriptName) {
        if (parsedPackage.kind === 'this') {
            return { ok: true, value: [contextScript.packageDir] };
        }
        if (parsedPackage.kind === 'path') {
            return this.#resolvePathPackage(packageJson, contextScript, dependencySpecifier, parsedPackage);
        }
        if (parsedPackage.kind === 'npm') {
            return await this.#resolveNpmPackage(packageJson, contextScript, dependencySpecifier, parsedPackage);
        }
        if (parsedPackage.kind === 'dependencies') {
            return await this.#resolveNpmDependencyPackages(packageJson, contextScript, dependencySpecifier, parsedPackage, scriptName);
        }
        if (parsedPackage.kind === 'workspaces') {
            return await this.#resolveWorkspacesPackages(packageJson, contextScript, dependencySpecifier, parsedPackage, scriptName);
        }
        parsedPackage;
        throw new Error(`Unexpected parsed package format: ${JSON.stringify(parsedPackage)}`);
    }
    #resolvePathPackage(packageJson, contextScript, dependencySpecifier, parsedPackage) {
        const absolute = pathlib.resolve(contextScript.packageDir, parsedPackage.path);
        if (absolute === contextScript.packageDir) {
            return invalidSyntaxError(`Cross-package dependency "${dependencySpecifier.value}" ` +
                `resolved to the same package.`, contextScript, packageJson.jsonFile, {
    ...
    async #resolveNpmPackage(packageJson, contextScript, dependencySpecifier, parsedPackage) {
        // TODO(aomarks) Support "dependencies".
        return invalidSyntaxError('NPM packages are not yet supported', contextScript, packageJson.jsonFile, {
            offset: dependencySpecifier.offset + 1 + parsedPackage.range.offset,
            length: parsedPackage.range.length,
        });
    }
    async #resolveNpmDependencyPackages(packageJson, contextScript, dependencySpecifier, parsedPackage, scriptName) {
        const localPackages = [];
        for (const dep of [
            ...
            let packageJsonPath;
            try {
                packageJsonPath = await findPackageJson(dep.value, contextScript.packageDir);
            }
            catch (e) {
                const maybeError = e;
                return invalidSyntaxError(`Could not resolve dependency ${JSON.stringify(dep.value)}` +
                    ` relative to ${JSON.stringify(contextScript.packageDir)}.` +
                    ` Is it installed? ` +
                    ('message' in maybeError ? `: ${maybeError.message}` : String(e)), contextScript, packageJson.jsonFile, {
                    offset: dep.offset,
                    length: dep.length,
                });
            }
        ...
        const localPackageJsons = await Promise.all(localPackages.map(async (packagePath) => [packagePath, await this.getPackageJson(packagePath)]));
    ...
    async #resolveWorkspacesPackages(packageJson, contextScript, dependencySpecifier, parsedPackage, scriptName) {
        if (packageJson.workspaces.length === 0) {
            return invalidSyntaxError(`No workspaces found in package.json`, contextScript, packageJson.jsonFile, {
                offset: dependencySpecifier.offset + 1 + parsedPackage.range.offset,
                length: parsedPackage.range.length,
            });
        }
        this.#resolvedWorkspacesConfigs.push({
            ...
            patterns: packageJson.workspaces.map((pattern) => pathlib.posix.join(pattern, 'package.json')),
        });
        const workspaceGlobResults = await glob(packageJson.workspaces.map((pattern) => pathlib.posix.join(pattern, 'package.json')), {
            cwd: contextScript.packageDir,
 ...
 */
export const failUnlessArray = (astNode, file) => {
...
const invalidSyntaxError = (message, script, file, range) => ({
    ok: false,
    error: {
        type: 'failure',
        reason: 'invalid-config-syntax',
