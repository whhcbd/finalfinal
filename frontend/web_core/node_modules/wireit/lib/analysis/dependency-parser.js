...
class DependencyParser {
    ...
    parse() {
        ...
        if (this.#done()) {
            ...
            if (startsWithPeriod) {
                // E.g. "../foo".
                return this.#error(`Cross-package dependency must use syntax "<relative-path>#<script-name>", ` +
    ...
    #parseSegment(inPathPosition) {
        ...
        while (!this.#done()) {
            ...
            if (cur === LT) {
                ...
                return this.#error(String.raw `Unexpected "<". Escape as "\<" if you meant it literally.`, { offset: this.#pos, length: 1 });
            }
            if (cur === BACKSLASH) {
    ...
    #parseSpecial() {
        ...
        while (!this.#done()) {
            ...
            if (cur === GT) {
                if (special.length === 0) {
                    return this.#error(String.raw `Unexpected ">". Escape as "\>" if you meant it literally.`, { offset: start, length: 2 });
                }
                this.#skip();
                return {
                    ok: true,
    ...
    #interpretSegmentAsPackage(segment) {
    ...
    #interpretSegmentAsScript(segment) {
        if (segment.kind === 'string') {
            if (segment.string.length === 0) {
                return this.#error(`Cross-package dependency must use syntax "<relative-path>#<script-name>", ` +
                    `but there's no script name in ${JSON.stringify(this.#str)}.`, { offset: 0, length: this.#len });
            }
            return {
                ok: true,
                value: {
