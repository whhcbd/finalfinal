...
"""
    ...
    def _rule_based_intent(self, user_message: str) -> Optional[str]:
        ...
        rules = [
            # 中心法则：DNA复制、转录、翻译过程
            ('central_dogma',               r'中心法则|DNA.{0,5}复制.{0,10}(过程|机制|方式)|转录.{0,5}翻译|mRNA.{0,5}翻译|核糖体.{0,10}(翻译|合成蛋白)|RNA聚合酶|解旋酶|冈崎片段|半保留复制|反转录|tRNA.{0,15}(携带|运输|氨基酸|参与翻译)|以.{0,15}为模板.{0,10}(合成蛋白质|翻译|转录)|翻译.{0,10}(过程|模板|合成蛋白)|转录.{0,10}(过程|模板|生成RNA)|蛋白质.{0,5}(合成过程|生物合成)|展示.{0,5}(DNA复制|转录|翻译|中心法则)'),
            # 孟德尔模拟器
            ('mendel_simulator',             r'模拟.{0,10}(杂交|受精)|验证.{0,10}(比例|定律)|随机受精|卡方检验|孟德尔模拟'),
            # 自然选择模拟器
            ('natural_selection_simulator',  r'自然选择.{0,10}(模拟|演示|过程)|种群演化|基因频率.{0,10}(变化|计算)|选择压力|适应度|捕食者.{0,5}猎物|哈迪.?温伯格'),
            # 旁氏表/棋盘格：杂交遗传结果
            ('punnett_square',               r'旁氏表|旁氏图|孟德尔方格|[Pp]unnett|[a-z]{2}[×x×][a-z]{2}|杂交.{0,15}(后代|基因型|比例)|测交.{0,10}(后代|结果)|自交.{0,10}(后代|结果)|配子.{0,10}(组合|类型)'),
            # DNA结构
            ('dna_structure',                r'DNA.{0,5}(双螺旋|结构|碱基)|双螺旋.{0,10}(结构|模型)|碱基.{0,5}(配对|互补).{0,10}(规则|原则)|脱氧核糖|磷酸二酯键|DNA.{0,5}(碱基序列|核苷酸序列)'),
            # 染色体交叉互换
            ('cross_over_map',               r'交叉互换|基因连锁.{0,10}(图|分析)|重组频率|染色体.{0,5}交换|连锁.{0,10}基因.{0,10}(图|分析)'),
            # 家系遗传图
            ('pedigree_chart',               r'家系图|系谱图|遗传病.{0,10}(世代|家族|图谱)|X.{0,3}连锁.{0,5}(遗传|显性|隐性)|常染色体.{0,5}(显性|隐性).{0,5}遗传'),
            # 基因表达调控
            ('gene_expression',              r'基因.{0,5}(表达|调控).{0,5}(水平|机制|过程)|基因表达谱|启动子.{0,5}(结合|调控)|操纵子|转录.{0,5}(调控|因子)|mRNA.{0,5}(水平|含量|稳定性)|基因.{0,5}(开|关|激活|抑制)'),
            # 表型分布比例
            ('phenotype_distribution',       r'表型.{0,10}(分布|比例)|表现型.{0,10}(分布|比例)|[Ff][12]代.{0,10}(比例|分布)|9.?[：:]?.?3.?[：:]?.?3.?[：:]?.?1|显性.{0,5}隐性.{0,5}(比例|分离比)'),
            # 测验出题
